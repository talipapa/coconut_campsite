...
const MainHeader:React.FC<MainHeaderProps> = ({fullName}) => {
  return (
    <>
        <StatusBar barStyle="light-content" backgroundColor="#5CBCB6" />

        <View className=' min-h-[16vh] bg-[#5CBCB6] flex flex-row items-end justify-between px-5 pb-3'>
          <View className='w-full flex flex-row space-x-3 items-end justify-between'>
            ...
            <Image source={require('@/assets/logo.jpg')} className='w-10 h-10 rounded-full'/>
              ...
              <View className='flex flex-col items-start'>
                ...
                <Text className='text-white text-md font-semibold'>{fullName.length > 15 ? fullName.substring(0, 10) + '...' : fullName}</Text>
